      ...
      exhaustMap(([{ loadingKey }, shortFavorites]) => {
        return this.favoritesService.getDetailedFavorites(shortFavorites).pipe(
          map((detailedFavorites: IFavoriteDetailedInfo[]) => {
            return FavoritesActions.getDetailedFavoritesSuccess({
              detailedFavorites,
              loadingKey,
            });
          }),
          catchError((error: Error) => {
            return of(
              FavoritesActions.getDetailedFavoritesFailure({
                error: error.message,
                loadingKey,
              }),
            );
      ...
      switchMap(([{ shortFavorite }]) => {
        return this.favoritesService.getDetailedFavorite(shortFavorite).pipe(
          map((detailedFavorite: IFavoriteDetailedInfo) => {
            return FavoritesActions.addDetailedFavoriteSuccess({
              detailedFavorite,
            });
          }),
          catchError((error: Error) => {
            return of(
              FavoritesActions.addDetailedFavoriteFailure({
      ...
      exhaustMap(({ id, name }) => {
        return this.weatherService.getWeather(id).pipe(
          map((weather: IWeather) =>
            FavoritesActions.updateDetailedFavoriteSuccess({
              id,
              name,
              weather,
            }),
          ...
          catchError((error: Error) =>
            of(
              FavoritesActions.updateDetailedFavoriteFailure({
                id,
                name,
                error: error.message,
              }),
